...
int rayTriangleIntersection(CompFab::Ray &ray, CompFab::Triangle &triangle)
{
    ...
    CompFab::Vec3 e1, e2; // edges 


     e1 = (triangle.m_v2 - triangle.m_v1); //sets the edges
    ...
    det = e1*P; // if det is near 0 then the rays has crossed a plane
    ...
    // tests if the intersection is inside or outside the triangle
    u = indet * (T*P); 
    if(u < 0.f || u > 1.f){
        return 0;
    }
...
int numSurfaceIntersections(CompFab::Vec3 &voxelPos, CompFab::Vec3 &dir)
{
    ...
    CompFab::RayStruct mRay = CompFab::RayStruct(voxelPos, dir);

    //iterates through all triangles and checks if the ray intersects
    for(unsigned int i = 0; i < g_triangleList.size(); i++){
        CompFab::Triangle triangle = g_triangleList[i];
        if(rayTriangleIntersection(mRay, triangle) == 0){
            numHits ++;
        }
    }


    // check to see if the num hits are iterating
...
int main(int argc, char **argv)
{
    ...
    gridSpacing = g_voxelGrid->m_spacing; // spacing of the grid
    gridLLeft = g_voxelGrid->m_lowerLeft; 

    // variable to hold the coordinates of the grid
    x = g_voxelGrid->m_dimX;
    y = g_voxelGrid->m_dimY;
    z = g_voxelGrid->m_dimZ;


    // nested for loops to iterate through all of the the points in the grid
    for (int i = 0; i < x; i++) {
        ...
        for (int j = 0; j < y; j++) {
            ...
            for (int k = 0; k < z; k++) {
                ...
                CompFab::Vec3 vPos(gridLLeft.m_x + ((double)i)*gridSpacing, gridLLeft.m_y + ((double)j)*gridSpacing, gridLLeft.m_z +((double)k)*gridSpacing);
                
                // tests if the ray is crosses a surface
                if(numSurfaceIntersections(vPos, direction) % 2 != 0){
                    g_voxelGrid->isInside(i,j,k) = 1;
